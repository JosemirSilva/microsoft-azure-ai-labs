...
> O desafio baseia-se no **Vision Studio** e leitura de texto em imagens. Neste repositório usamos também **Portrait processing** (Vision Studio) e **Document Intelligence Studio** (OCR/Read) no mesmo ecossistema **Azure AI**, com recurso dedicado (ex.: `ai-dio-vision-dev-centralus` / variantes por região).
...
Explorar **detecção de rostos**, **leitura de texto (OCR)** e **metadados derivados de imagens** (atributos faciais, JSON, avisos de qualidade), transformando conteúdo visual em **dados estruturados** e documentando o processo.
...
**Nota:** *Se no futuro quiser reforçar o alinhamento exato com o link “Analyze images” do curso, você pode acrescentar uma captura da demo **Analyze image** no Vision Studio em `output/` e atualizar esta tabela.*
...
- [x] **`README.md`** com processo e insights (este arquivo).
...
- **Rostos em grupo e oclusão:** com rede ou multidão, o serviço pode marcar várias faces ou “occlusão”; útil para discutir **confiabilidade** e **contexto**.
...
Confirmar que **não** aparecem `.env`, chaves ou arquivos sensíveis.
...
Com ramo **main** (ou **master**, conforme o seu remoto):
...
1. Crie um repositório vazio no GitHub (sem README, se já tem um local).
...
*(Substitua `SEU_USUARIO` e o nome do repo pelos seus valores reais.)*

### 6. Depois do push

1. Abra o repositório no browser e confirme que `labs/azure-ml-face-images/output/` mostra os PNG e o `ocr_propaganda1.txt`.
