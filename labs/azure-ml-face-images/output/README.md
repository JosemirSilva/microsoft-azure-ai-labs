...
| Tipo | Exemplo de ficheiro |
|------|---------------------|
| Texto (OCR) | `ocr_<nome-da-imagem>.txt` ou `.json` com o texto detetado |
...
Se o Vision Studio só permitir **captura de ecrã**, pode duplicar os prints em [`assets/screenshots/`](../assets/screenshots/) e no `README.md` principal mencionar que o `output/` contém exports equivalentes quando disponíveis.

## Alguns ficheiros nesta pasta

| Ficheiro | Conteúdo |
|----------|-----------|
| `imagem1.png` … `imagem5.png` | Capturas da demo **Detect faces** no Vision Studio. |
| `imagem6-portrait-processing.png` | Demo **Portrait processing**: rosto detetado, painel JSON (pose, blur, máscara, qualidade) e aviso de rosto pouco adequado a remoção de fundo — evidência de “imagem → dados” e limites de qualidade. |
