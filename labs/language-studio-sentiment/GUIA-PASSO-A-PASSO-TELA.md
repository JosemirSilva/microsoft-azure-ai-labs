...
Use este ficheiro como **roteiro linear** no browser. Os nomes exatos dos botões podem variar ligeiramente (idioma da UI ou atualização Microsoft); em caso de dúvida, siga primeiro as **video-aulas da DIO** e use a **pesquisa** dentro do Language Studio.
...
1. Abra uma janela anónima ou normal e vá a **[https://language.cognitive.azure.com/](https://language.cognitive.azure.com/)**.
2. Clique em **Iniciar sessão** / **Sign in** e use a **mesma conta** da subscrição Azure onde está o recurso.
...
## Parte C — Associar subscrição e recurso

1. Deverá surgir um ecrã para **escolher a subscrição** (**Subscription**). Selecione a subscrição correta (ex.: “Azure subscription 1”).
...
4. **Captura:** guarde como [`output/language-studio-home.png`](./output/language-studio-home.png) — deve mostrar o **ecrã principal** do Language Studio com o recurso já ligado (cabeçalho ou rodapé por vezes indica o nome do recurso).

Se **não** aparecer o recurso na lista: no Portal, confirme que a implementação está concluída e que a tua conta tem permissão de leitura no resource group.

---

## Parte D — Abrir a análise de sentimentos

1. Na **página inicial** do Language Studio, percorra os **cartões** ou o **menu lateral** à procura de funcionalidades de texto semelhantes a:
...
3. Se a UI pedir **confirmação** de que a demo consome quota do teu recurso, marque a caixa e continue (é o equivalente ao Vision Studio “I acknowledge…”).
...
1. Na área de **entrada de texto** / **Enter text** / caixa grande, cole uma frase **claramente positiva** em português. Exemplo (podes alterar):  
...
1. Cole uma frase **neutra** (facto, horário, sem elogio nem reclamação), por exemplo:  
...
3. Se preferir, faça print do painel JSON em **`sentiment-results-json.png`** em vez de ficheiro `.json`.
...
2. Inicie sessão, escolha recurso de **Speech** (podes precisar de criar um recurso **Speech** se ainda não existir — seguir o Learn “Explore Speech Studio”).
3. Documente com capturas `output/speech-<descricao>.png` e uma frase curta no [README](./README.md) na secção de extensão.

---

## Parte K — Fechar o lab no repositório

1. Confirme os ficheiros em [`output/`](./output/) (nomes sugeridos em [`output/README.md`](./output/README.md)).
...
|-----------|-------------|
| Pedido de recurso e lista vazia | Criar/implementar recurso Language no Portal; aguardar alguns minutos. |
| Erro de região ou funcionalidade indisponível | Ver documentação da funcionalidade para a tua região; ou criar recurso noutra região suportada (avaliar custo). |
| Tudo “neutro” | Testar frases mais explícitas emocionalmente; registar nos Insights como limitação do modelo ou do texto. |
| Interface noutra língua | Alterar idioma do browser ou da conta Microsoft, ou seguir posição dos botões com este guia em paralelo. |
